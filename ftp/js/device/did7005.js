...
function _playerRecordingStart(name, url){
	Platform.Recording.startRecording(__playerStripToken(url), name).then(function(result){
		recURL = result;
	});
};

function _playerRecordingStop(){
	Platform.Recording.stopRecording(recURL);
};

function _playerRecordingGet(){
	var 
		recordings = [],
		list = Platform.Recording.getRecordingsListSync();
	
	for (var i = 0; i < list.length; i++)
	{
		recordings.push({
			rec_date: new Date(list[i].timeStart).toString(),
			rec_name: list[i].metadata.replaceAll('_', '-'),
			rec_url: list[i].mrl,
			rec_image: ''
		});
	}
...
function _playerRecordingDelete(name, url){
	Platform.Recording.removeRecordingSync(url);
};
