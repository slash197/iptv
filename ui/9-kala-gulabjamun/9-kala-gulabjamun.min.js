GWareIPTV.UIPrototype.prototype.updateTime = function () {
    ...
    for (var t = this, a = App.epg.getProgrammeByTimestamp(e.id, e.ts), s = this.checkProgDate(a), i = $('<div class="preview" />'), r = "", n = 0; n < a.channel.data.length; n++) r = a.ut_start === a.channel.data[n].ut_start && null !== a.channel.data[n].progimg ? '<img src="' + a.channel.data[n].progimg + '" />' : "";
    $(".preview").length && this.closePreview(), setTimeout(function () {
        i.css($(".epgh .selection-bg").offset()).html('<div class="details"><span class="name">' + a.name + '</span><span class="time">' + new Date(1e3 * a.ut_start).niceTime() + '</span><div class="description" style="white-space:normal;">' + r + a.description + '</div></div><div class="buttons nav" />'), $("body").append(i), s.push({
            ...
            selection: "bg"
        }), t.disableZones(".buttons"), t.object = $(".buttons .item:first-child"), t.select()
    }, 100), App.EPGh.keyBack = App.EPGh.closePreview
}, App.EPGh.checkProgDate = function (e) {
    var t = new Date(1e3 * e.ut_start), a = new Date(1e3 * e.ut_end), s = new Date, i = {
        "data-title": e.name,
    ...
    var a = [], s = [], t = [], i = [], e = App.history.getSeriesUnique();
    App.metro.movies.forEach(function (e) {
        ...
        s.push({
            html: '<div class="img" style="background: url(' + t.seasons[0].backdrop + ');"></div><div class="details row-fluid"><div class="name span6">' + t.name + '</div><div class="title span6"></div><div class="progress"><div class="bar" style="width: 0%"></div></div></div>',
        ...
        i.push({
            ...
            style: {height: $(".channels-holder").height(), width: $(".channels-holder").height()}
        })
    }), this.makeZone({items: a, selector: ".movies", selection: "border"}), this.makeZone({
        items: s,
        selector: ".series",
        selection: "bg"
    }), this.makeZone({
        ...
        selector: ".channels",
        selection: "border"
    }), App.user.device.hasApps ? (App.metro.apps.forEach(function (e) {
        t.push({html: '<img src="' + App.settings.server_location.app_location + '/images/apps/' + e.icon + '" />', data: {"data-id": e.id}})
    }), 0 < t.length ? this.makeZone({
        items: t,
        selector: ".apps"
    }) : ($(".channels-holder .span6:last-child").remove(), $(".channels-holder .span6").addClass("span12").removeClass("span6"))) : ($(".channels-holder .span6:last-child").remove(), $(".channels-holder .span6").addClass("span12").removeClass("span6")), this.index = 0, this.timerNews = null, $(".movies-holder img, .series-holder img, .channels-holder img").waitForImages(function () {
        App.showPage()
    })
